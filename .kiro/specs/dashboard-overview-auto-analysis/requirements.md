...
本文档定义了 Burp Copilot 项目的两个核心增强功能：**Dashboard 概览页面**和**自动分析规则引擎**。Dashboard 概览页面为 Web 前端提供安全态势总览，用户打开仪表盘即可看到分析统计、严重性分布和最近发现。自动分析规则引擎使 Burp 扩展能够根据用户配置的作用域规则自动提交匹配的 HTTP 流量进行分析，无需手动干预。
